...
export function HomeTechnicalSection() {
  ...
  const copy = {
    zh: {
      ...
      subtitle: '普通站点走 Worker SSE 流式反馈; 复杂 SPA 触发 Browser Run 云端渲染,由 Workflows 编排整站抓取，产物落 R2 后交付下载。',
      pipeline: [
        ['01', 'URL Intake', '校验协议、规范化目标 URL、生成任务上下文。'],
        ['02', 'Edge Crawl', 'Worker 拉取页面和资源,实时向浏览器推送进度。'],
        ['03', 'Render Escalation', '检测 JS 依赖时切换 Browser Run 云端渲染任务。'],
    ...
    en: {
      ...
      subtitle: 'Standard sites use Worker SSE streaming. Complex SPAs trigger Browser Run cloud rendering, orchestrated by Workflows, with artifacts delivered from R2.',
      pipeline: [
        ...
        ['03', 'Render Escalation', 'Detected JS dependencies move the job to a Browser Run cloud render.'],
      ...
      matrix: [
        ...
        ['SPA dynamic content', 'Browser Run fallback'],
